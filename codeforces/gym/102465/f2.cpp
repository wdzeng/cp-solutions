...
bool cmp(const pair<int, ll> l, const pair<int, ll> r) {
    return pos(tv[l.first].first, tv[l.first].second) == pos(tv[r.first].first, tv[r.first].second) ? tv[l.first].second * tv[r.first].first < tv[r.first].second * tv[l.first].first : pos(tv[l.first].first, tv[l.first].second) < pos(tv[r.first].first, tv[r.first].second);
}
    ...
    for (int x = 0; x < n; x++) {
        ...
        for (int i = 1; i < n; i++) {
            ...
            if (tv[s[i].first].second == 0) {
                if (tv[s[i].first].first > 0)
                    tts[i].second = 0.0;
                else
                    tts[i].second = M_PI;
            } else
                tts[i].second = atan2(tv[s[i].first].second, tv[s[i].first].first);
